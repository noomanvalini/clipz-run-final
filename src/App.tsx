...
import { Play, Pause, Square, Share2, MapPin, Activity, Timer, Navigation, Sparkles, Camera, Footprints, History, LogOut, ChevronLeft, Save, User as UserIcon, Lock, X } from 'lucide-react';
...
const generateGeminiContent = async (prompt: string): Promise<string> => {
    ...
    try {
        ...
        return data.candidates?.[0]?.content?.parts?.[0]?.text || "Treino concluído!";
    } catch (error) {
...
// --- Container Base ---
const MobileContainer = ({ children, className }: { children: React.ReactNode, className?: string }) => {
    return (
        <div className="min-h-screen w-full flex items-center justify-center bg-black/95 p-0 md:p-8 font-sans">
            <div className={`w-full max-w-[420px] md:aspect-[9/16] h-full md:h-auto min-h-screen md:min-h-0 relative overflow-hidden md:shadow-2xl md:rounded-[30px] flex flex-col ${className}`}
...
export default function App() {
    ...
    useEffect(() => {
        if (status === 'finished' && !aiCaption && !loadingAiCaption) {
            generateCaption(distance, elapsedTime, calculatePaceVal(distance, elapsedTime), activityType);
        }
    }, [status]);
    ...
    const generateCaption = async (d: number, t: number, p: string, type: string) => {
        ...
        // Correção: Agora usamos 't' e 'p' no prompt para evitar erro TS6133
        const prompt = `Acabei de finalizar uma ${activityName}. Estatísticas: ${d.toFixed(2)}km, ${formatTime(t)} de duração e pace de ${p}. Crie uma legenda curta e vibrante para Instagram Stories com emojis.`;
    ...
    const startSimulation = () => {
        setStatus('running');
        let lat = -23.587416, lng = -46.657634, angle = 0;
        updatePositionLogic([lat, lng]);
        timerId.current = setInterval(() => setElapsedTime(p => p + 1), 1000);
        simulationInterval.current = setInterval(() => {
            angle += 0.08; lat += Math.sin(angle) * 0.00015; lng += Math.sin(angle * 2) * 0.00015;
            updatePositionLogic([lat, lng]);
        }, 1000);
    };
        ...
        return (
            <MobileContainer className="p-8 justify-center">
                <div className="flex flex-col items-center mb-8">
                    <img src={LOGO_SYMBOL} className="w-20 h-20 mb-4 object-contain" onError={(e) => e.currentTarget.style.display = 'none'} />
                    <img src={LOGO_FULL} className="h-8 mb-2 object-contain" />
                    <p className="text-xs text-center opacity-60" style={{ color: theme.textMuted }}>O combustível de quem não para</p>
                </div>

                <div className="bg-white/5 p-6 rounded-2xl w-full border border-white/10">
                    <h2 className="text-white font-bold text-xl mb-6 text-center">{isLogin ? 'Login' : 'Criar Conta'}</h2>
                    <form onSubmit={handleAuth} className="space-y-4">
                        <div className="flex items-center bg-black/30 rounded-lg px-3 py-3 border border-white/10 focus-within:border-orange-500">
                            ...
                            <input type="email" required value={email} onChange={e => setEmail(e.target.value)}
                                className="bg-transparent text-white w-full outline-none text-sm" placeholder="Email" />
                        </div>
                        <div className="flex items-center bg-black/30 rounded-lg px-3 py-3 border border-white/10 focus-within:border-orange-500">
                            <Lock size={16} className="text-slate-500 mr-2" />
                            <input type="password" required value={password} onChange={e => setPassword(e.target.value)}
                                className="bg-transparent text-white w-full outline-none text-sm" placeholder="Senha" />
                        </div>
                    ...
                    <button onClick={() => setIsLogin(!isLogin)} className="mt-4 text-xs text-slate-400 hover:text-white w-full text-center">
